export const IC_THREE_DOT = require('./icons/ThreeDot.png');
export const IC_ADD = require('./icons/Add.png');
export const IC_SEARCH = require('./icons/Search.png');
export const IC_BACK = require('./icons/Back.png');
export const IC_UNCHECK_RADIO = require('./icons/ic_uncheck_radio.png');
export const IC_CHECK_RADIO = require('./icons/ic_check_radio.png');
export const IC_EMAIL = require('./icons/ic_email.png');
export const IC_SMS = require('./icons/ic_sms.png');
export const IC_POST_LIKE = require('./icons/IC_POST_LIKE.png');
export const IC_POST_COMMENT = require('./icons/IC_POST_COMMENT.png');
export const IC_POST_MESSAGE = require('./icons/IC_POST_MESSAGE.png');

export const IMG_LOGO = require('./images/Logo.png');
export const IMG_AVATAR = require('./images/Profile.png');
export const IMG_ILLUSTRATION = require('./images/Illustration.png');
